...
def run():
    with grpc.insecure_channel("localhost:6012") as channel:
        ...
        def on_release(key):
            if key == Key.space:
                stub.SendMessageToTopic(api_pb2_grpc.api__pb2.PublishPayload(topic=api_pb2_grpc.api__pb2.Channel(channelId="16Uiu2HAm7WTnfH5GLtFVTPMc79Qu8TzMoEKe4QEDnWiSBRjr8UZf"),message=api_pb2_grpc.api__pb2.Msg(payload=str.encode("HELLO"))))
                print("SPACE")
            if key == Key.enter:
